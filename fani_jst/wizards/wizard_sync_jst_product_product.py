...
class WizardSyncJSTProductTemplate(models.TransientModel):
    _name = 'wizard.sync.jst.product.product'
    _description = 'Wizard Sync JST Product'
    ...
    def action_sync(self):
        ...
        while modifiedEnd >= date_from_tp:
            ...
            if modifiedBegin < date_from_tp:
                modifiedBegin = date_from_tp
            time.sleep(1)
            self.env['jst.product.product'].sudo().action_sync_jst_products(modifiedBegin, modifiedEnd)
